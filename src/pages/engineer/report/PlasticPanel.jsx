...
const PlasticPanel = ({ data = {}, onChange }) => {
  // Issues state initialization
  const [condition, setCondition] = useState(() => {
    const init = {};
    plasticPanels.forEach((panel) => {
      init[panel] = data?.[`plastic_${panel}_issues`] || "None";
    });
    return init;
  });

  // Photos state initialization
  ...
  const handleConditionChange = (panel, val) => {
    setCondition((prev) => {
      ...
      onChange?.(`plastic_${panel}_issues`, val);
      return updated;
    });
  };
  ...
  const updatePhotos = (panel, newPhotos) => {
    setPhotos((prev) => {
      ...
      onChange?.(`plastic_${panel}_imageUrls`, newPhotos);
      return updated;
    });
  };

  // Toggle third row enabled state and propagate onChange
  ...
  if (!photos || Object.keys(photos).length === 0) {
    return <div className="text-white">Loading plastic panel data...</div>;
  }

  return (
    <div className="bg-[#ffffff0a] backdrop-blur-[16px] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.2)] w-full max-w-4xl mx-auto text-white">
      <h2 className="text-2xl sm:text-3xl font-heading mb-6 sm:mb-8 text-white text-left">Plastic Panels</h2>
