...
const [popupMessage, setPopupMessage] = useState("");
  ...
  const handleNextWithSaveConfirm = () => {
  setShowPopup(true); // open popup instead of window.confirm
};
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-primary">
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 sm:px-6 md:px-12 py-8 sm:py-12">
        ...
        {/* Navigation */}
        {/* Popup Message (top of page) */}
{showPopup && (
  <div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center gap-4">
    <span>{popupMessage}</span>
    <button
      onClick={() => {
        setShowPopup(false);

        // If popup is for submit, trigger handleSave after confirmation
        if (popupMessage.includes("Submitted")) {
          handleSave(isAdm ? null : APPLICATION_CONSTANTS.REQUEST_STATUS.WAITING_FOR_APPROVAL.value, true);
        }

        // If popup is for save (next), trigger handleNextWithSaveConfirm after confirmation
        if (popupMessage.includes("Saved")) {
          handleNextWithSaveConfirm();
        }
      }}
      className="ml-4 px-4 py-1 bg-white text-green-600 rounded-md hover:bg-gray-100"
    >
      OK
    </button>
  </div>
...
<div className="flex flex-col sm:flex-row justify-between mt-8 sm:mt-12 w-full max-w-4xl gap-4 sm:gap-0 px-2">
  ...
  {step < steps.length - 1 ? (
    ...
    : ""
      ...
      {showPopup && (
    <div className="fixed inset-0 flex items-start justify-center  bg-opacity-40 backdrop-blur-sm z-50">
    <div className="bg-white p-6 rounded-xl shadow-lg text-center relative w-80 mt-10">
