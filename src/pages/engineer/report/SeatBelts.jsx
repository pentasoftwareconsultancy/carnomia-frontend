...
const SeatBelts = ({ seatBeltDetails, setSeatBeltDetails, showPhoto, setShowPhoto }) => {
  ...
  useEffect(() => {
    const updated = seatBeltPanels.reduce(
      (acc, id) => {
    ...
    setStreamStates(updated.streamStates);
  ...
  const renderPanel = (id, idx) => {
    const isThirdRow = id === 'thirdRow';
    const showContent = isThirdRow ? thirdRowEnabled : true;

    return (
      <div key={id} className="flex flex-col w-full">
        <div className="flex justify-between items-center mb-2">
          <label className="text-md text-white font-medium text-left">
            {`${idx + 1}. ${capitalizeFirstWord(id.replace(/([A-Z])/g, ' $1'))}`}
          </label>
          {isThirdRow && (
            <ToggleButton checked={thirdRowEnabled} onChange={() => setThirdRowEnabled((prev) => !prev)} />
          )}
        </div>
  ...
  return (
    <div className="bg-[#ffffff0a] backdrop-blur-[16px] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.2)] w-full max-w-4xl mx-auto text-white">
      ...
      <div className="grid grid-cols-1 gap-6 sm:gap-8">
        {seatBeltPanels.map((id, idx) => renderPanel(id, idx))}
      </div>
