...
const StyledCard = styled(Card)(({ theme }) => ({
  ...
  width: '100%',
  transition: 'none',
  // Removed hover effect completely for all devices
}));
...
const JobsManagement = ({ requests, setRequests, setSelectedRequest, setViewMode, setEditMode, assignedJobsFilter, setAssignedJobsFilter, assignedJobsRef }) => {
              ...
              {columns.map((column) => (
                <StyledTableCell key={column} sx={{ fontWeight: 'bold', color: '#2E7D32' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {column === 'Booking ID' && <FiTool size={16} style={{ marginRight: 4 }} />}
                    {column === 'Customer' && <FiUser size={16} style={{ marginRight: 4 }} />}
                    {column === 'Date' && <FiCalendar size={16} style={{ marginRight: 4 }} />}
                    {column}
                  </Box>
                </StyledTableCell>
              ))}
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <StyledButton
                ...
                sx={{
                  backgroundColor: assignedJobsFilter === 'assigned' ? '#FFA000' : 'transparent',
                  color: assignedJobsFilter === 'assigned' ? '#ffffff' : '#FFA000',
                  borderColor: '#FFA000',
                  '&:hover': {
                    backgroundColor: assignedJobsFilter === 'assigned' ? '#F57C00' : '#FFF8E1',
                    color: assignedJobsFilter === 'assigned' ? '#ffffff' : '#F57C00',
                  },
                }}
              >
              ...
              <StyledButton
                ...
                sx={{
                  backgroundColor: assignedJobsFilter === 'unassigned' ? '#4CAF50' : 'transparent',
                  color: assignedJobsFilter === 'unassigned' ? '#ffffff' : '#4CAF50',
                  borderColor: '#4CAF50',
                  '&:hover': {
                    backgroundColor: assignedJobsFilter === 'unassigned' ? '#388E3C' : '#E8F5E9',
                    color: assignedJobsFilter === 'unassigned' ? '#ffffff' : '#388E3C',
                  },
                }}
              >
                <FiFilter size={16} style={{ marginRight: 4 }} />
